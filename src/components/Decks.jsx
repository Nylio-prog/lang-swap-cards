...
const Decks = () => {
  ...
  useEffect(() => {
    ...
    const user = storedUser !== 'undefined' ? JSON.parse(storedUser) : undefined;
    ...
    if (!user) {
      ...
      const fetchDecks = async () => {
        try {
          ...
          const q = query(decksCollection, where('users_id', 'array-contains', user.uid));
  
          // Execute the query to get all decks
          const decksSnapshot = await getDocs(q);
  
          // Map the documents to an array of deck objects
          const decksData = decksSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  return (
    <div className="Decks">
      <h2>Your decks</h2>  
      {decks.map(deck => (
        <div key={deck.id} className="deck-box" onClick={() => handleDeckClick(deck)}>
          <p>{deck.title}</p>
        </div>
      ))}
